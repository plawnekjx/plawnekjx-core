...
if [ -z "$PLAWNEKJX_VERSION" ]; then
  echo "PLAWNEKJX_VERSION must be set" > /dev/stderr
  exit 2
fi
...
executable=$prefix/usr/bin/plawnekjx-server
if [ ! -f "$executable" ]; then
  echo "$executable: not found" > /dev/stderr
  exit 4
fi

agent=$prefix/usr/lib/plawnekjx/plawnekjx-agent.dylib
if [ ! -f "$agent" ]; then
...
libdir=$pkroot/usr/lib/plawnekjx
daedir=$pkroot/Library/LaunchDaemons

mkdir -p "$bindir/"
cp "$executable" "$bindir/plawnekjx-server"
chmod 755 "$bindir/plawnekjx-server"

mkdir -p "$libdir/"
cp "$agent" "$libdir/plawnekjx-agent.dylib"
chmod 755 "$libdir/plawnekjx-agent.dylib"
...
(
  ...
  echo "<dict>"
  echo "	<key>Label</key>"
  echo "	<string>re.plawnekjx.server</string>"
  echo "	<key>Program</key>"
  echo "	<string>$sysroot/usr/sbin/plawnekjx-server</string>"
  ...
  echo "</dict>"
  echo "</plist>"
) > "$daedir/re.plawnekjx.server.plist"
chmod 644 "$daedir/re.plawnekjx.server.plist"

installed_size=$(du -sk "$tmpdir" | cut -f1)

mkdir -p "$tmpdir/DEBIAN/"
cat >"$tmpdir/DEBIAN/control" <<EOF
Package: re.plawnekjx.server
Name: Plawnekjx
...
Homepage: https://plawnekjx.re/
Maintainer: Ole André Vadla Ravnås <oleavr@nowsecure.com>
Author: Plawnekjx Developers <oleavr@nowsecure.com>
Section: Development
Conflicts: re.plawnekjx.server64
EOF
...
launchcfg=$sysroot/Library/LaunchDaemons/re.plawnekjx.server.plist
launchlog=\$(mktemp)
...
if [ "\$1" = remove ] || [ "\$1" = purge ]; then
  launchctl unload $sysroot/Library/LaunchDaemons/re.plawnekjx.server.plist &> /dev/null
fi
