[CCode (gir_namespace = "PlawnekjxDroidyInjector", gir_version = "1.0")]
namespace Plawnekjx.Droidy.Injector {
	...
	private sealed class Session : Object {
		...
		private async GadgetDetails inject_gadget (Cancellable? cancellable) throws Error, IOError {
			...
			string so_path_shared = "/data/android/plawnekjx/plawnekjx-gadget-" + instance_id + ".so";
			string so_path_app = "/data/data/" + package + "/gadget.so";
			string config_path_shared = "/data/android/plawnekjx/plawnekjx-gadget-" + instance_id + ".config";
			string config_path_app = "/data/data/" + package + "/gadget.config";
			string unix_socket_path = "plawnekjx:" + package;
