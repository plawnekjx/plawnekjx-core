namespace Plawnekjx {
	...
	private sealed class HelperFactory {
		...
		public async LinuxHelper obtain (Cancellable? cancellable) throws Error, IOError {
			...
			try {
				string socket_path = "/plawnekjx-" + Uuid.string_random ();
	...
	private sealed class ResourceStore {
		...
		public ResourceStore (TemporaryDirectory tempdir) throws Error {
			...
			var blob32 = Plawnekjx.Data.Helper.get_plawnekjx_helper_32_blob ();
			if (blob32.data.length > 0)
				helper32 = make_temporary_helper ("plawnekjx-helper-32", blob32.data);

			var blob64 = Plawnekjx.Data.Helper.get_plawnekjx_helper_64_blob ();
			if (blob64.data.length > 0)
				helper64 = make_temporary_helper ("plawnekjx-helper-64", blob64.data);
#else
			var tpl = PathTemplate (Config.PLAWNEKJX_HELPER_PATH);
