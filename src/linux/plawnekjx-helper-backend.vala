namespace Plawnekjx {
	...
	private sealed class InjectSession : SeizeSession {
		...
		public async RemoteAgent inject (InjectSpec spec, Cancellable? cancellable) throws Error, IOError {
			...
			try {
				unowned uint8[] loader_code = Plawnekjx.Data.HelperBackend.get_loader_bin_blob ().data;
		...
		private LoaderLayout compute_loader_layout (InjectSpec spec, string fallback_address) {
			...
			unowned uint8[] code = Plawnekjx.Data.HelperBackend.get_loader_bin_blob ().data;
		...
		private async RemoteAgent launch_loader (LoaderLaunch launch, InjectSpec spec, BootstrapResult bres,
			...
			if (loader_result.status != COMPLETED) {
				...
				if (pc >= loader_base && pc < loader_base + Plawnekjx.Data.HelperBackend.get_loader_bin_blob ().data.length) {
		...
		private async BootstrapResult bootstrap (size_t loader_size, Cancellable? cancellable) throws Error, IOError {
			...
			unowned uint8[] bootstrapper_code = Plawnekjx.Data.HelperBackend.get_bootstrapper_bin_blob ().data;
		...
		private static string make_fallback_address () {
			return "/plawnekjx-" + Uuid.string_random ();
		}
	...
	private sealed class RemoteAgent : Object {
		...
		public UnixConnection plawnekjx_ctrl {
			get;
			construct;
		}
		...
		private RemoteAgent (uint pid, InjectSpec spec, BootstrapResult bres, UnixConnection plawnekjx_ctrl,
				UnixConnection? agent_ctrl = null) {
			Object (
				pid: pid,
				inject_spec: spec,
				bootstrap_result: bres,
				plawnekjx_ctrl: plawnekjx_ctrl,
		...
		internal static async RemoteAgent start (LoaderLaunch launch, InjectSpec spec, uint pid, BootstrapResult bres,
				UnixConnection plawnekjx_ctrl, UnixConnection? agent_ctrl, Cancellable? cancellable) throws Error, IOError {
			var agent = new RemoteAgent (pid, spec, bres, plawnekjx_ctrl, agent_ctrl);

			try {
				var io_priority = Priority.DEFAULT;

				if (launch == FROM_SCRATCH)
					plawnekjx_ctrl.send_fd (spec.library_so.get_fd (), cancellable);

				if (agent.agent_ctrlfd_for_peer != null) {
					plawnekjx_ctrl.send_fd (agent.agent_ctrlfd_for_peer.handle, cancellable);
					agent.agent_ctrlfd_for_peer = null;
				} else {
					yield plawnekjx_ctrl.get_output_stream ().write_async ({ 0 }, io_priority, cancellable);
				}
			} catch (GLib.Error e) {
				if (e is IOError.CANCELLED)
					throw (IOError) e;
		...
		public void ack () {
			...
			plawnekjx_ctrl.get_output_stream ().write_all_async.begin ((uint8[]) &raw_type, Priority.DEFAULT, null);
		}
		...
		private async void monitor () {
			...
			try {
				...
				InputStream input = plawnekjx_ctrl.get_input_stream ();
				var io_priority = Priority.DEFAULT;
				size_t n;
		...
		public RemoteAgent clone (uint clone_id, AgentFeatures features) {
			var agent = new RemoteAgent (0, inject_spec.clone (clone_id, features), bootstrap_result.clone (), plawnekjx_ctrl);
			agent._state = _state;
			return agent;
		}
		...
		switch (err) {
			...
			case Posix.EPERM:
				throw new Error.PERMISSION_DENIED (
					...
					" try `sudo sysctl kernel.yama.ptrace_scope=0`, or run Plawnekjx as root",
					pid);
			default:
