...
typedef struct _PlawnekjxMacModel PlawnekjxMacModel;

struct _PlawnekjxMacModel
{
  const gchar * name;
  const gchar * icon;
};

static const PlawnekjxMacModel mac_models[] =
{
...
GVariant *
_plawnekjx_darwin_host_session_provider_try_extract_icon (void)
{
#ifdef HAVE_MACOS
  GVariant * icon;
  size_t size;
  gchar * model_name;
  const PlawnekjxMacModel * model;
  guint i;
  gchar * filename;
