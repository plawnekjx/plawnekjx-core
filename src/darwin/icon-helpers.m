...
GVariant *
_plawnekjx_icon_from_file (const gchar * filename, guint target_width, guint target_height)
{
#ifdef HAVE_MACOS
  ...
  if (image != nil)
  {
    result = _plawnekjx_icon_from_native_image_scaled_to (image, target_width, target_height);
    [image release];
  }
...
GVariant *
_plawnekjx_icon_from_native_image_scaled_to (PlawnekjxNativeImage native_image, guint target_width, guint target_height)
{
  GVariant * result;
#ifdef HAVE_MACOS
   ...
   * those usually rendered onto by the Plawnekjx GUI... ICK!
   */
