namespace Plawnekjx {
	public sealed class DarwinHelperProcess : Object, DarwinHelper {
		...
		private static void throw_helper_error (GLib.Error e) throws Error, IOError {
			...
			if (e is IOError.CLOSED) {
				throw new Error.PERMISSION_DENIED ("Oops, plawnekjx-helper appears to have crashed. It may have been killed " +
					...
					"tss_should_crash=0\"`. For more information, see: https://github.com/plawnekjx/plawnekjx-core/issues/524");
			}
#endif
	...
	private sealed class ResourceStore {
		...
		public ResourceStore (TemporaryDirectory tempdir) throws Error {
			...
			var blob = Plawnekjx.Data.Helper.get_plawnekjx_helper_blob ();
			helper = new TemporaryHelperFile (
				new TemporaryFile.from_stream ("plawnekjx-helper",
					new MemoryInputStream.from_data (blob.data, null),
					tempdir));
			FileUtils.chmod (helper.path, 0700);
#else
			helper = new InstalledHelperFile.for_path (Config.PLAWNEKJX_HELPER_PATH);
#endif
		}
