#include "plawnekjx-helper-process-glue.h"
...
void *
plawnekjx_helper_factory_spawn (const gchar * path, const gchar * parameters, PlawnekjxPrivilegeLevel level, GError ** error)
{
  ...
  if (level == PLAWNEKJX_PRIVILEGE_LEVEL_ELEVATED)
    ei.lpVerb = L"runas";
  else
    ei.lpVerb = L"open";
...
gboolean
plawnekjx_helper_instance_is_process_still_running (void * handle)
{
  DWORD exit_code;
...
void
plawnekjx_helper_instance_close_process_handle (void * handle)
{
