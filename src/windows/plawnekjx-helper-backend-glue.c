...
typedef struct _PlawnekjxInjectionDetails PlawnekjxInjectionDetails;
typedef struct _PlawnekjxRemoteWorkerContext PlawnekjxRemoteWorkerContext;
...
struct _PlawnekjxInjectionDetails
{
  HANDLE process_handle;
...
struct _PlawnekjxRemoteWorkerContext
{
...
static void plawnekjx_propagate_open_process_error (guint32 pid, DWORD os_error, GError ** error);
static gboolean plawnekjx_enable_debug_privilege (void);

static gboolean plawnekjx_remote_worker_context_init (PlawnekjxRemoteWorkerContext * rwc, PlawnekjxInjectionDetails * details, GError ** error);
static gsize plawnekjx_remote_worker_context_emit_payload (PlawnekjxRemoteWorkerContext * rwc, gpointer code);
static void plawnekjx_remote_worker_context_destroy (PlawnekjxRemoteWorkerContext * rwc, PlawnekjxInjectionDetails * details);

static gboolean plawnekjx_remote_worker_context_has_resolved_all_kernel32_functions (const PlawnekjxRemoteWorkerContext * rwc);
static gboolean plawnekjx_remote_worker_context_collect_kernel32_export (const GumExportDetails * details, gpointer user_data);

static gboolean plawnekjx_file_exists_and_is_readable (const WCHAR * filename);

void
_plawnekjx_windows_helper_backend_inject_library_file (guint32 pid, const gchar * path, const gchar * entrypoint, const gchar * data,
    void ** inject_instance, void ** waitable_thread_handle, GError ** error)
{
  gboolean success = FALSE;
  const gchar * failed_operation;
  NTSTATUS nt_status;
  PlawnekjxInjectionDetails details;
  DWORD desired_access;
  HANDLE thread_handle = NULL;
  gboolean rwc_initialized = FALSE;
  PlawnekjxRemoteWorkerContext rwc;
  ...
  if (!plawnekjx_file_exists_and_is_readable (details.dll_path))
    goto invalid_path;

  plawnekjx_enable_debug_privilege ();
  ...
  if (!plawnekjx_remote_worker_context_init (&rwc, &details, error))
    goto beach;
  ...
  instance->stay_resident_address = (guint8 *) rwc.argument + G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, stay_resident);
...
os_failure:
  {
    ...
    {
      plawnekjx_propagate_open_process_error (pid, os_error, error);
    }
    else
    {
      g_set_error (error,
          PLAWNEKJX_ERROR,
          (os_error == ERROR_ACCESS_DENIED) ? PLAWNEKJX_ERROR_PERMISSION_DENIED : PLAWNEKJX_ERROR_NOT_SUPPORTED,
...
nt_failure:
  {
    ...
    if (nt_status == 0xC0000022) /* STATUS_ACCESS_DENIED */
      code = PLAWNEKJX_ERROR_PERMISSION_DENIED;
    else
      code = PLAWNEKJX_ERROR_NOT_SUPPORTED;
...
beach:
  {
    if (!success && rwc_initialized)
      plawnekjx_remote_worker_context_destroy (&rwc, &details);
...
void
_plawnekjx_windows_helper_backend_free_inject_instance (void * inject_instance, gboolean * is_resident)
{
  PlawnekjxInjectInstance * instance = inject_instance;
  gboolean stay_resident;
  SIZE_T n_bytes_read;
...
static void
plawnekjx_propagate_open_process_error (guint32 pid, DWORD os_error, GError ** error)
{
...
static gboolean
plawnekjx_enable_debug_privilege (void)
{
...
static gboolean
plawnekjx_remote_worker_context_init (PlawnekjxRemoteWorkerContext * rwc, PlawnekjxInjectionDetails * details, GError ** error)
{
  gpointer code;
  guint code_size;
  GumModule * kernel32;
  ...
  code_size = plawnekjx_remote_worker_context_emit_payload (rwc, code);

  memset (rwc, 0, sizeof (PlawnekjxRemoteWorkerContext));

  kernel32 = gum_process_find_module_by_name ("kernel32.dll");
  gum_module_enumerate_exports (kernel32, plawnekjx_remote_worker_context_collect_kernel32_export, rwc);
  g_object_unref (kernel32);
  if (!plawnekjx_remote_worker_context_has_resolved_all_kernel32_functions (rwc))
  ...
  alloc_size = page_size + sizeof (PlawnekjxRemoteWorkerContext);
  ...
  if (!WriteProcessMemory (details->process_handle, rwc->argument, rwc, sizeof (PlawnekjxRemoteWorkerContext), NULL))
    goto write_process_memory_failed;
...
#define EMIT_ARM64_LOAD(reg, field) \
    gum_arm64_writer_put_ldr_reg_reg_offset (&cw, ARM64_REG_##reg, ARM64_REG_X20, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, field))
#define EMIT_ARM64_LOAD_ADDRESS_OF(reg, field) \
    gum_arm64_writer_put_add_reg_reg_imm (&cw, ARM64_REG_##reg, ARM64_REG_X20, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, field))
...
static gsize
plawnekjx_remote_worker_context_emit_payload (PlawnekjxRemoteWorkerContext * rwc, gpointer code)
{
  gsize code_size;
  ...
  /* x20 = (PlawnekjxRemoteWorkerContext *) lpParameter */
  ...
  /* xbx = (PlawnekjxRemoteWorkerContext *) lpParameter */
  ...
  gum_x86_writer_put_lea_reg_reg_offset (&cw, GUM_X86_XCX,
      GUM_X86_XBX, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, dll_path));
  gum_x86_writer_put_call_reg_offset_ptr_with_arguments (&cw, GUM_CALL_SYSAPI,
      GUM_X86_XBX, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, load_library_impl),
      1,
  ...
  gum_x86_writer_put_lea_reg_reg_offset (&cw, GUM_X86_XDX,
      GUM_X86_XBX, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, entrypoint_name));
  gum_x86_writer_put_call_reg_offset_ptr_with_arguments (&cw, GUM_CALL_SYSAPI,
      GUM_X86_XBX, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, get_proc_address_impl),
      2,
  ...
  gum_x86_writer_put_lea_reg_reg_offset (&cw, GUM_X86_XCX,
      GUM_X86_XBX, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, entrypoint_data));
  gum_x86_writer_put_lea_reg_reg_offset (&cw, GUM_X86_XDX,
      GUM_X86_XBX, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, stay_resident));
  ...
  /* FreeLibrary (xsi) */
  gum_x86_writer_put_call_reg_offset_ptr_with_arguments (&cw, GUM_CALL_SYSAPI,
      GUM_X86_XBX, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, free_library_impl),
      1,
      GUM_ARG_REGISTER, GUM_X86_XSI);
  ...
  /* result = GetLastError() */
  gum_x86_writer_put_call_reg_offset_ptr_with_arguments (&cw, GUM_CALL_SYSAPI,
      GUM_X86_XBX, G_STRUCT_OFFSET (PlawnekjxRemoteWorkerContext, get_last_error_impl),
      0);
...
static void
plawnekjx_remote_worker_context_destroy (PlawnekjxRemoteWorkerContext * rwc, PlawnekjxInjectionDetails * details)
{
  if (rwc->entrypoint != NULL)
  {
...
static gboolean
plawnekjx_remote_worker_context_has_resolved_all_kernel32_functions (const PlawnekjxRemoteWorkerContext * rwc)
{
...
static gboolean
plawnekjx_remote_worker_context_collect_kernel32_export (const GumExportDetails * details, gpointer user_data)
{
  PlawnekjxRemoteWorkerContext * rwc = user_data;
...
static gboolean
plawnekjx_file_exists_and_is_readable (const WCHAR * filename)
{
  HANDLE file;
