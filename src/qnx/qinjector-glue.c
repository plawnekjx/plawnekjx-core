...
#define PLAWNEKJX_REMOTE_PAYLOAD_SIZE (8192)
#define PLAWNEKJX_REMOTE_DATA_OFFSET (512)
#define PLAWNEKJX_REMOTE_STACK_OFFSET (PLAWNEKJX_REMOTE_PAYLOAD_SIZE - 512)
#define PLAWNEKJX_REMOTE_DATA_FIELD(n) \
    remote_address + PLAWNEKJX_REMOTE_DATA_OFFSET + G_STRUCT_OFFSET (PlawnekjxTrampolineData, n)

typedef struct _PlawnekjxInjectionInstance PlawnekjxInjectionInstance;
typedef struct _PlawnekjxInjectionParams PlawnekjxInjectionParams;
typedef struct _PlawnekjxCodeChunk PlawnekjxCodeChunk;
typedef struct _PlawnekjxTrampolineData PlawnekjxTrampolineData;
typedef struct _PlawnekjxFindLandingStripContext PlawnekjxFindLandingStripContext;

typedef void (* PlawnekjxEmitFunc) (const PlawnekjxInjectionParams * params, GumAddress remote_address, PlawnekjxCodeChunk * code);

struct _PlawnekjxInjectionInstance
{
  PlawnekjxQinjector * qinjector;
  guint id;
  pid_t pid;
...
struct _PlawnekjxInjectionParams
{
  pid_t pid;
  const gchar * so_path;
...
struct _PlawnekjxFindLandingStripContext
{
  pid_t pid;
  GumAddress result;
};

static gboolean plawnekjx_emit_and_remote_execute (PlawnekjxEmitFunc func, const PlawnekjxInjectionParams * params, GumAddress * result, GError ** error);

static void plawnekjx_emit_payload_code (const PlawnekjxInjectionParams * params, GumAddress remote_address, PlawnekjxCodeChunk * code);

static GumAddress plawnekjx_remote_alloc (pid_t pid, size_t size, int prot, GError ** error);
static int plawnekjx_remote_dealloc (pid_t pid, GumAddress address, size_t size, GError ** error);
static int plawnekjx_remote_pthread_create (pid_t pid, GumAddress address, GError ** error);
static int plawnekjx_remote_msync (pid_t pid, GumAddress remote_address, gint size, gint flags, GError ** error);
static gboolean plawnekjx_remote_write (pid_t pid, GumAddress remote_address, gconstpointer data, gsize size, GError ** error);
static gboolean plawnekjx_remote_write_fd (gint fd, GumAddress remote_address, gconstpointer data, gsize size, GError ** error);
static gboolean plawnekjx_remote_call (pid_t pid, GumAddress func, const GumAddress * args, gint args_length, GumAddress * retval, GError ** error);

static GumAddress plawnekjx_resolve_remote_libc_function (int remote_pid, const gchar * function_name);

static GumAddress plawnekjx_resolve_remote_library_function (int remote_pid, const gchar * library_name, const gchar * function_name);
static GumAddress plawnekjx_find_library_base (pid_t pid, const gchar * library_name, gchar ** library_path);

static PlawnekjxInjectionInstance *
plawnekjx_injection_instance_new (PlawnekjxQinjector * qinjector, guint id, pid_t pid, const char * temp_path)
{
  PlawnekjxInjectionInstance * instance;

  instance = g_slice_new0 (PlawnekjxInjectionInstance);
...
static void
plawnekjx_injection_instance_free (PlawnekjxInjectionInstance * instance, PlawnekjxUnloadPolicy unload_policy)
{
  if (instance->remote_payload != 0 && unload_policy == PLAWNEKJX_UNLOAD_POLICY_IMMEDIATE)
  {
    GError * error = NULL;

    plawnekjx_remote_dealloc (instance->pid, instance->remote_payload, PLAWNEKJX_REMOTE_PAYLOAD_SIZE, &error);
  ...
  g_slice_free (PlawnekjxInjectionInstance, instance);
}

void
_plawnekjx_remote_thread_session_receive_pulse (void * opaque_instance, PlawnekjxQnxPulseCode * code, gint * val, GError ** error)
{
  PlawnekjxInjectionInstance * instance = opaque_instance;
  int res;
  struct _pulse pulse;
...
gboolean
_plawnekjx_remote_thread_session_thread_is_alive (guint pid, guint tid)
{
...
void
_plawnekjx_qinjector_free_instance (PlawnekjxQinjector * self, void * instance, PlawnekjxUnloadPolicy unload_policy)
{
  plawnekjx_injection_instance_free (instance, unload_policy);
}

guint
_plawnekjx_qinjector_do_inject (PlawnekjxQinjector * self, guint pid, const gchar * path, const gchar * entrypoint, const gchar * data,
    const gchar * temp_path, GError ** error)
{
  PlawnekjxInjectionInstance * instance;
  PlawnekjxInjectionParams params = { pid, path, entrypoint, data };

  instance = plawnekjx_injection_instance_new (self, self->next_instance_id++, pid, temp_path);

  params.channel_id = instance->channel_id;
  params.remote_address = plawnekjx_remote_alloc (pid, PLAWNEKJX_REMOTE_PAYLOAD_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC, error);
  if (params.remote_address == 0)
    goto beach;
  instance->remote_payload = params.remote_address;

  if (!plawnekjx_emit_and_remote_execute (plawnekjx_emit_payload_code, &params, NULL, error))
    goto beach;
...
beach:
  {
    plawnekjx_injection_instance_free (instance, PLAWNEKJX_UNLOAD_POLICY_IMMEDIATE);
    return 0;
  }
}

static gboolean
plawnekjx_emit_and_remote_execute (PlawnekjxEmitFunc func, const PlawnekjxInjectionParams * params, GumAddress * result,
    GError ** error)
{
  ...
  data = (PlawnekjxTrampolineData *) (code.bytes + PLAWNEKJX_REMOTE_DATA_OFFSET);
  ...
  if (!plawnekjx_remote_write (params->pid, params->remote_address, code.bytes, PLAWNEKJX_REMOTE_DATA_OFFSET + sizeof (PlawnekjxTrampolineData), error))
    return FALSE;
   ...
   */
  if (plawnekjx_remote_msync (params->pid, params->remote_address, PLAWNEKJX_REMOTE_PAYLOAD_SIZE, MS_INVALIDATE_ICACHE, error) != 0)
    return FALSE;

  if (plawnekjx_remote_pthread_create (params->pid, params->remote_address, error) != 0)
    return FALSE;
...
#define EMIT_LOAD_FIELD(reg, field) \
    gum_arm_writer_put_ldr_reg_address (&cw, ARM_REG_##reg, PLAWNEKJX_REMOTE_DATA_FIELD (field)); \
    EMIT_LDR (reg, reg)
#define EMIT_STORE_FIELD(field, reg) \
    gum_arm_writer_put_ldr_reg_address (&cw, ARM_REG_R0, PLAWNEKJX_REMOTE_DATA_FIELD (field)); \
...
static void
plawnekjx_emit_payload_code (const PlawnekjxInjectionParams * params, GumAddress remote_address, PlawnekjxCodeChunk * code)
{
  GumArmWriter cw;
  ...
  EMIT_CALL_IMM (plawnekjx_resolve_remote_libc_function (params->pid, "ConnectAttach_r"),
      5,
  ...
  gum_arm_writer_put_call_address_with_arguments (&cw, plawnekjx_resolve_remote_libc_function (params->pid, "gettid"), 0);
  EMIT_MOVE (R3, R0);

  EMIT_CALL_IMM (plawnekjx_resolve_remote_libc_function (params->pid, "MsgSendPulse_r"),
      4,
      ARG_REG (R7),
      ARG_IMM (-1),
      ARG_IMM (PLAWNEKJX_QNX_PULSE_CODE_HELLO),
      ARG_REG (R3));
  ...
  {
    EMIT_CALL_IMM (plawnekjx_resolve_remote_libc_function (params->pid, "dlopen"),
        2,
  ...
  EMIT_CALL_IMM (plawnekjx_resolve_remote_libc_function (params->pid, "dlsym"),
      2,
      ARG_REG (R6),
      ARG_IMM (PLAWNEKJX_REMOTE_DATA_FIELD (entrypoint_name)));
  gum_arm_writer_put_mov_reg_reg (&cw, ARM_REG_R5, ARM_REG_R0);

  EMIT_LDR_U32 (R0, PLAWNEKJX_UNLOAD_POLICY_IMMEDIATE);
  ...
  EMIT_LDR (R0, SP);
  EMIT_CMP (R0, PLAWNEKJX_UNLOAD_POLICY_IMMEDIATE);
  EMIT_B_COND (NE, skip_dlclose);
  {
    EMIT_CALL_IMM (plawnekjx_resolve_remote_libc_function (params->pid, "dlclose"),
        1,
        ARG_REG (R6));
  }
  ...
  EMIT_LDR (R0, SP);
  EMIT_CMP (R0, PLAWNEKJX_UNLOAD_POLICY_DEFERRED);
  EMIT_B_COND (EQ, skip_detach);
  {
    EMIT_LOAD_FIELD (R0, worker_thread);
    EMIT_CALL_IMM (plawnekjx_resolve_remote_libc_function (params->pid, "pthread_detach"),
        1,
        ARG_REG (R0));
  }
  ...
  EMIT_CALL_IMM (plawnekjx_resolve_remote_libc_function (params->pid, "MsgSendPulse_r"),
      ...
      ARG_REG (R7),
      ARG_IMM (-1),
      ARG_IMM (PLAWNEKJX_QNX_PULSE_CODE_BYE),
      ARG_REG (R3));

  EMIT_CALL_IMM (plawnekjx_resolve_remote_libc_function (params->pid, "ConnectDetach_r"),
      1,
      ARG_REG (R7));
...
static GumAddress
plawnekjx_remote_alloc (pid_t pid, size_t size, int prot, GError ** error)
{
  GumAddress args[] = {
    0,
    size,
  ...
  GumAddress retval = 0;
  GumAddress function = plawnekjx_resolve_remote_libc_function (pid, "mmap");
  ...
  if (!plawnekjx_remote_call (pid, function, args, G_N_ELEMENTS (args), &retval, error))
    return 0;
...
static int
plawnekjx_remote_dealloc (pid_t pid, GumAddress address, size_t size, GError ** error)
{
  GumAddress args[] = {
    address,
    size
  ...
  GumAddress retval;
  GumAddress function = plawnekjx_resolve_remote_libc_function (pid, "munmap");
...
static int
plawnekjx_remote_pthread_create (pid_t pid, GumAddress remote_address, GError ** error)
{
  GumAddress args[] = {
    PLAWNEKJX_REMOTE_DATA_FIELD (worker_thread),
    0,
    remote_address,
    0
  };
  GumAddress retval;
  GumAddress function = plawnekjx_resolve_remote_libc_function (pid, "pthread_create");
...
static int
plawnekjx_remote_msync (pid_t pid, GumAddress remote_address, gint size, gint flags, GError ** error)
{
  GumAddress args[] = {
    remote_address,
    size,
  ...
  GumAddress retval;
  GumAddress function = plawnekjx_resolve_remote_libc_function (pid, "msync");
...
plawnekjx_remote_write (pid_t pid, GumAddress remote_address, gconstpointer data, gsize size, GError ** error)
{
  ...
  result = plawnekjx_remote_write_fd (fd, remote_address, data, size, error);
...
static gboolean
plawnekjx_remote_write_fd (gint fd, GumAddress remote_address, gconstpointer data, gsize size, GError ** error)
{
  long ret;
...
static gboolean
plawnekjx_remote_call (pid_t pid, GumAddress func, const GumAddress * args, gint args_length, GumAddress * retval, GError ** error)
{
  ...
  for (i = args_length - 1; i >= 4; i--)
  {
    ...
    if (!plawnekjx_remote_write_fd (fd, modified_registers.arm.gpr[GUM_QNX_ARM_REG_SP], &args[i],
        4, error))
      goto beach;
  }
...
static GumAddress
plawnekjx_resolve_remote_libc_function (int remote_pid, const gchar * function_name)
{
  return plawnekjx_resolve_remote_library_function (remote_pid, "libc", function_name);
}

static GumAddress
plawnekjx_resolve_remote_library_function (int remote_pid, const gchar * library_name, const gchar * function_name)
{
  ...
  local_base = plawnekjx_find_library_base (getpid (), library_name, &local_library_path);
  g_assert (local_base != 0);

  remote_base = plawnekjx_find_library_base (remote_pid, library_name, &remote_library_path);
  if (remote_base == 0)
  {
...
static GumAddress
plawnekjx_find_library_base (pid_t pid, const gchar * library_name, gchar ** library_path)
{
  GumAddress result = 0;
  gchar * as_path = NULL;
  int fd = -1;
