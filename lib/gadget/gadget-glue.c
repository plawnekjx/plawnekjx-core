...
#ifdef HAVE_DARWIN
static void plawnekjx_parse_apple_parameters (const gchar * apple[], gboolean * found_range, GumMemoryRange * range, gchar ** config_data);
#endif
...
plawnekjx_on_load (int argc, const char * argv[], const char * envp[], const char * apple[], int * result)
{
  gboolean found_range;
  GumMemoryRange range;
  gchar * config_data;

  plawnekjx_parse_apple_parameters (apple, &found_range, &range, &config_data);

  plawnekjx_gadget_load (found_range ? &range : NULL, config_data, (config_data != NULL) ? result : NULL);
...
__attribute__ ((constructor)) static void
plawnekjx_on_load (void)
{
  plawnekjx_gadget_load (NULL, NULL, NULL);
}

__attribute__ ((destructor)) static void
plawnekjx_on_unload (void)
{
  plawnekjx_gadget_unload ();
}
...
void
plawnekjx_gadget_environment_init (void)
{
  gum_init_embedded ();
  gio_init ();

  g_thread_set_garbage_handler (_plawnekjx_gadget_on_pending_thread_garbage, NULL);
  ...
  plawnekjx_error_quark (); /* Initialize early so GDBus will pick it up */
  ...
  worker_thread = g_thread_new("thread-913", run_worker_loop, NULL);
}

void
plawnekjx_gadget_environment_deinit (void)
{
  GSource * source;
  ...
  plawnekjx_run_atexit_handlers ();

#ifdef HAVE_DARWIN
  /* Do what plawnekjx_deinit_memory() does on the other platforms. */
  gum_internal_heap_unref ();
#endif
}

gboolean
plawnekjx_gadget_environment_can_block_at_load_time (void)
{
#ifdef HAVE_WINDOWS
  return FALSE;
#else
...
GumThreadId
plawnekjx_gadget_environment_get_worker_tid (void)
{
  return worker_tid;
}

GMainContext *
plawnekjx_gadget_environment_get_worker_context (void)
{
...
void
plawnekjx_gadget_log_info (const gchar * message)
{
  g_info ("%s", message);
}

void
plawnekjx_gadget_log_warning (const gchar * message)
{
  g_warning ("%s", message);
}
...
static void
plawnekjx_parse_apple_parameters (const gchar * apple[], gboolean * found_range, GumMemoryRange * range, gchar ** config_data)
{
  const gchar * entry;
  guint i = 0;
  ...
  *config_data = NULL;
  ...
  {
    if (g_str_has_prefix (entry, "plawnekjx_dylib_range="))
    {
      *found_range = sscanf (entry, "plawnekjx_dylib_range=0x%" G_GINT64_MODIFIER "x,0x%" G_GSIZE_MODIFIER "x",
          &range->base_address, &range->size) == 2;
    }
    else if (g_str_has_prefix (entry, "plawnekjx_gadget_config="))
    {
      guchar * data;
      gsize size;
