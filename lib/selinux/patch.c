...
#define PLAWNEKJX_SELINUX_ERROR plawnekjx_selinux_error_quark ()

typedef struct _PlawnekjxSELinuxRule PlawnekjxSELinuxRule;
typedef enum _PlawnekjxSELinuxErrorEnum PlawnekjxSELinuxErrorEnum;

struct _PlawnekjxSELinuxRule
{
...
enum _PlawnekjxSELinuxErrorEnum
{
  PLAWNEKJX_SELINUX_ERROR_POLICY_FORMAT_NOT_SUPPORTED,
  PLAWNEKJX_SELINUX_ERROR_TYPE_NOT_FOUND,
  PLAWNEKJX_SELINUX_ERROR_CLASS_NOT_FOUND,
  PLAWNEKJX_SELINUX_ERROR_PERMISSION_NOT_FOUND
};

static gboolean plawnekjx_load_policy (const gchar * filename, policydb_t * db, gchar ** data, GError ** error);
static gboolean plawnekjx_save_policy (const gchar * filename, policydb_t * db, GError ** error);
static type_datum_t * plawnekjx_ensure_type (policydb_t * db, const gchar * type_name, guint num_attributes, ...);
static void plawnekjx_add_type_to_class_constraints_referencing_attribute (policydb_t * db, uint32_t type_id, uint32_t attribute_id);
static gboolean plawnekjx_ensure_permissive (policydb_t * db, const gchar * type_name, GError ** error);
static avtab_datum_t * plawnekjx_ensure_rule (policydb_t * db, const gchar * s, const gchar * t, const gchar * c, const gchar * p, GError ** error);

static gboolean plawnekjx_set_file_contents (const gchar * filename, const gchar * contents, gssize length, GError ** error);

static const PlawnekjxSELinuxRule plawnekjx_selinux_rules[] =
{
  { { "domain", NULL }, "domain", "process", { "execmem", NULL } },
  { { "domain", NULL }, "plawnekjx_file", "dir", { "search", NULL } },
  { { "domain", NULL }, "plawnekjx_file", "file", { "open", "read", "getattr", "execute", "?map", NULL } },
  { { "domain", NULL }, "plawnekjx_memfd", "file", { "open", "read", "write", "getattr", "execute", "?map", NULL } },
...
G_DEFINE_QUARK (plawnekjx-selinux-error-quark, plawnekjx_selinux_error)

void
plawnekjx_selinux_patch_policy (void)
{
  ...
  if (!plawnekjx_load_policy (system_policy, &db, &db_data, &error))
  {
  ...
  if (plawnekjx_ensure_type (&db, "plawnekjx_file", 2, "file_type", "mlstrustedobject", &error) == NULL)
  {
  ...
  if (plawnekjx_ensure_type (&db, "plawnekjx_memfd", 2, "file_type", "mlstrustedobject", &error) == NULL)
  {
    g_printerr ("Unable to add SELinux type: %s\n", error->message);
    g_clear_error (&error);
    goto beach;
  ...
  for (rule_index = 0; rule_index != G_N_ELEMENTS (plawnekjx_selinux_rules); rule_index++)
  {
    const PlawnekjxSELinuxRule * rule = &plawnekjx_selinux_rules[rule_index];
    ...
    for (source_cursor = rule->sources; *source_cursor != NULL; source_cursor++)
    {
      ...
      for (perm_entry = rule->permissions; *perm_entry != NULL; perm_entry++)
      {
        ...
        if (plawnekjx_ensure_rule (&db, source, target, rule->klass, perm, &error) == NULL)
        {
          if (!g_error_matches (error, PLAWNEKJX_SELINUX_ERROR, PLAWNEKJX_SELINUX_ERROR_PERMISSION_NOT_FOUND) || is_important)
  ...
  if (!plawnekjx_save_policy ("/sys/fs/selinux/load", &db, &error))
  {
    ...
    if (probably_in_emulator)
    {
      ...
      success = plawnekjx_ensure_permissive (&db, "shell", &error);
      if (success)
        success = plawnekjx_save_policy ("/sys/fs/selinux/load", &db, &error);
...
static gboolean
plawnekjx_load_policy (const gchar * filename, policydb_t * db, gchar ** data, GError ** error)
{
  policy_file_t file;
  int res;
  ...
  *data = file.data;
  ...
  if (res != 0)
  {
    g_set_error (error, PLAWNEKJX_SELINUX_ERROR, PLAWNEKJX_SELINUX_ERROR_POLICY_FORMAT_NOT_SUPPORTED, "unsupported policy database format");
    policydb_destroy (db);
    g_free (*data);
    return FALSE;
  }
...
static gboolean
plawnekjx_save_policy (const gchar * filename, policydb_t * db, GError ** error)
{
  void * data;
  size_t size;
  int res G_GNUC_UNUSED;
  ...
  return plawnekjx_set_file_contents (filename, data, size, error);
}

static type_datum_t *
plawnekjx_ensure_type (policydb_t * db, const gchar * type_name, guint n_attributes, ...)
{
  type_datum_t * type;
  uint32_t type_id;
  va_list vl;
  ...
  for (i = 0; i != n_attributes; i++)
  {
    ...
    if (attribute_type != NULL)
    {
      ...
      plawnekjx_add_type_to_class_constraints_referencing_attribute (db, type_id, attribute_id);
    }
    else if (pending_error == NULL)
    {
      g_set_error (&pending_error, PLAWNEKJX_SELINUX_ERROR, PLAWNEKJX_SELINUX_ERROR_TYPE_NOT_FOUND, "attribute type “%s” does not exist", attribute_name);
    }
  }
...
static void
plawnekjx_add_type_to_class_constraints_referencing_attribute (policydb_t * db, uint32_t type_id, uint32_t attribute_id)
{
...
static gboolean
plawnekjx_ensure_permissive (policydb_t * db, const gchar * type_name, GError ** error)
{
  type_datum_t * type;
  int res G_GNUC_UNUSED;
  ...
  if (type == NULL)
  {
    g_set_error (error, PLAWNEKJX_SELINUX_ERROR, PLAWNEKJX_SELINUX_ERROR_TYPE_NOT_FOUND, "type %s does not exist", type_name);
    return FALSE;
  }
...
plawnekjx_ensure_rule (policydb_t * db, const gchar * s, const gchar * t, const gchar * c, const gchar * p, GError ** error)
{
  ...
  if (source == NULL)
  {
    g_set_error (error, PLAWNEKJX_SELINUX_ERROR, PLAWNEKJX_SELINUX_ERROR_TYPE_NOT_FOUND, "source type “%s” does not exist", s);
    return NULL;
  }
  ...
  if (target == NULL)
  {
    g_set_error (error, PLAWNEKJX_SELINUX_ERROR, PLAWNEKJX_SELINUX_ERROR_TYPE_NOT_FOUND, "target type “%s” does not exist", t);
    return NULL;
  }
  ...
  if (klass == NULL)
  {
    g_set_error (error, PLAWNEKJX_SELINUX_ERROR, PLAWNEKJX_SELINUX_ERROR_CLASS_NOT_FOUND, "class “%s” does not exist", c);
    return NULL;
  }
  ...
  if (perm == NULL)
  {
    g_set_error (error, PLAWNEKJX_SELINUX_ERROR, PLAWNEKJX_SELINUX_ERROR_PERMISSION_NOT_FOUND, "perm “%s” does not exist on the “%s” class", p, c);
    return NULL;
  }
...
static gboolean
plawnekjx_set_file_contents (const gchar * filename, const gchar * contents, gssize length, GError ** error)
{
  int fd, res;
  gsize offset, size;
