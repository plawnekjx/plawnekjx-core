...
static gchar * plawnekjx_steal_bio_to_string (BIO ** bio);

static int plawnekjx_on_connection_output (void * addr, void * buffer, size_t length, uint8_t tos, uint8_t set_df);
static void plawnekjx_on_debug_printf (const char * format, ...);

static void plawnekjx_on_upcall (struct socket * sock, void * user_data, int flags);

void
_plawnekjx_generate_certificate (guint8 ** cert_der, gint * cert_der_length, gchar ** cert_pem, gchar ** key_pem)
{
  X509 * x509;
  X509_NAME * name;
  EVP_PKEY * pkey;
  ...
  X509_NAME_add_entry_by_txt (name, "O", MBSTRING_ASC, (const unsigned char *) "Plawnekjx", -1, -1, 0);
  ...
  *cert_pem = plawnekjx_steal_bio_to_string (&bio);

  bio = BIO_new (BIO_s_mem ());
  PEM_write_bio_PrivateKey (bio, pkey, NULL, NULL, 0, NULL, NULL);
  *key_pem = plawnekjx_steal_bio_to_string (&bio);
...
static gchar *
plawnekjx_steal_bio_to_string (BIO ** bio)
{
  gchar * result;
  long n;
  char * str;
...
void
_plawnekjx_sctp_connection_initialize_sctp_backend (void)
{
  const int msec_per_sec = 1000;

  usrsctp_init_nothreads (0, plawnekjx_on_connection_output, plawnekjx_on_debug_printf);
...
static int
plawnekjx_on_connection_output (void * addr, void * buffer, size_t length, uint8_t tos, uint8_t set_df)
{
  PlawnekjxSctpConnection * connection = addr;

  _plawnekjx_sctp_connection_emit_transport_packet (connection, buffer, (gint) length);

  return 0;
}

static void
plawnekjx_on_debug_printf (const char * format, ...)
{
  g_printerr ("[SCTP] %s\n", format);
}

void *
_plawnekjx_sctp_connection_create_sctp_socket (PlawnekjxSctpConnection * self)
{
  struct socket * sock;
  ...
  usrsctp_set_upcall (sock, plawnekjx_on_upcall, self);
...
void
_plawnekjx_sctp_connection_connect_sctp_socket (PlawnekjxSctpConnection * self, void * sock, guint16 port)
{
...
static void
plawnekjx_on_upcall (struct socket * sock, void * user_data, int flags)
{
  PlawnekjxSctpConnection * connection = user_data;

  _plawnekjx_sctp_connection_on_sctp_socket_events_changed (connection);
}

void
_plawnekjx_sctp_connection_close (void * sock)
{
  usrsctp_close (sock);
}

void
_plawnekjx_sctp_connection_shutdown (void * sock, PlawnekjxSctpShutdownType type, GError ** error)
{
...
GIOCondition
_plawnekjx_sctp_connection_query_sctp_socket_events (void * sock)
{
...
void
_plawnekjx_sctp_connection_handle_transport_packet (PlawnekjxSctpConnection * self, guint8 * data, gint data_length)
{
  usrsctp_conninput (self, data, data_length, 0);
}

gssize
_plawnekjx_sctp_connection_recv (void * sock, guint8 * buffer, gint buffer_length, guint16 * stream_id, PlawnekjxPayloadProtocolId * protocol_id,
    PlawnekjxSctpMessageFlags * message_flags, GError ** error)
{
  gssize n;
  ...
  {
    *stream_id = 0;
    *protocol_id = PLAWNEKJX_PAYLOAD_PROTOCOL_ID_NONE;
  }

  *message_flags = 0;

  if ((msg_flags & MSG_EOR) != 0)
    *message_flags |= PLAWNEKJX_SCTP_MESSAGE_FLAGS_END_OF_RECORD;

  if ((msg_flags & MSG_NOTIFICATION) != 0)
    *message_flags |= PLAWNEKJX_SCTP_MESSAGE_FLAGS_NOTIFICATION;
...
gssize
_plawnekjx_sctp_connection_send (void * sock, guint16 stream_id, PlawnekjxPayloadProtocolId protocol_id, guint8 * data, gint data_length,
      GError ** error)
{
  gssize n;
...
gint
_plawnekjx_sctp_timer_source_get_timeout (void)
{
  return usrsctp_get_timeout ();
}

void
_plawnekjx_sctp_timer_source_process_timers (guint32 elapsed_msec)
{
