...
typedef struct _PlawnekjxInitMessage PlawnekjxInitMessage;

struct _PlawnekjxInitMessage
{
...
gint
_plawnekjx_darwin_pipe_consume_stashed_file_descriptor (const gchar * address, GError ** error)
{
  gint fd = -1;
  G_GNUC_UNUSED gint assigned;
  mach_port_t port = MACH_PORT_NULL;
  PlawnekjxInitMessage init = { { 0, }, { 0, } };
  kern_return_t kr;
  const gchar * failed_operation;
  mach_port_t wrapper;
