namespace Plawnekjx.InjectorTest {
	public static void add_tests () {
		...
		GLib.Test.add_func ("/Injector/suspended-injection-current-arch", () => {
			test_suspended_injection (Plawnekjx.Test.Arch.CURRENT);
		});

		if (can_test_cross_arch_injection) {
			GLib.Test.add_func ("/Injector/suspended-injection-other-arch", () => {
				test_suspended_injection (Plawnekjx.Test.Arch.OTHER);
			});
		}
#endif
	}

	private static void test_dynamic_injection (Plawnekjx.Test.Arch arch) {
		var logfile = File.new_for_path (Plawnekjx.Test.path_to_temporary_file ("dynamic-injection.log"));
		try {
			logfile.delete ();
		} catch (GLib.Error delete_error) {
		}
		var envp = new string[] {
			"PLAWNEKJX_LABRAT_LOGFILE=" + logfile.get_path ()
		};
		...
		switch (Plawnekjx.Test.os ()) {
			case Plawnekjx.Test.OS.MACOS:   // Gum.Darwin.Mapper
			case Plawnekjx.Test.OS.IOS:     // Gum.Darwin.Mapper
			case Plawnekjx.Test.OS.TVOS:    // Gum.Darwin.Mapper
			case Plawnekjx.Test.OS.ANDROID: // Bionic's behavior
				assert_true (content_of (logfile) == ">m<>m");
				break;
			case Plawnekjx.Test.OS.LINUX:
				if (Plawnekjx.Test.libc () == Plawnekjx.Test.Libc.UCLIBC) {
					assert_true (content_of (logfile) == ">m<>m");
				} else {
					assert_true (content_of (logfile) == ">m<>m<");
				}
	...
	private static void test_resident_injection (Plawnekjx.Test.Arch arch) {
		var logfile = File.new_for_path (Plawnekjx.Test.path_to_temporary_file ("resident-injection.log"));
		try {
			logfile.delete ();
		} catch (GLib.Error delete_error) {
		}
	...
	private static void test_resource_leaks () {
		var logfile = File.new_for_path (Plawnekjx.Test.path_to_temporary_file ("leaks.log"));
		var envp = new string[] {
			"PLAWNEKJX_LABRAT_LOGFILE=" + logfile.get_path ()
		};
...
#if DARWIN
	private static void test_suspended_injection (Plawnekjx.Test.Arch arch) {
		var logfile = File.new_for_path (Plawnekjx.Test.path_to_temporary_file ("suspended-injection.log"));
		try {
			logfile.delete ();
		} catch (GLib.Error delete_error) {
		}
	...
	private sealed class Labrat {
		public Plawnekjx.Test.Process? process {
			get;
			private set;
		}
		...
		public Labrat (string name, string[] envp, Plawnekjx.Test.Arch arch = Plawnekjx.Test.Arch.CURRENT) {
			try {
				process = Plawnekjx.Test.Process.start (Plawnekjx.Test.Labrats.path_to_executable (name), null, envp, arch);
			} catch (Error e) {
				printerr ("\nFAIL: %s\n\n", e.message);
				assert_not_reached ();
			}
		...
		public Labrat.suspended (string name, string[] envp, Plawnekjx.Test.Arch arch = Plawnekjx.Test.Arch.CURRENT) {
			try {
				process = Plawnekjx.Test.Process.create (Plawnekjx.Test.Labrats.path_to_executable (name), null, envp, arch);
			} catch (Error e) {
				printerr ("\nFAIL: %s\n\n", e.message);
				assert_not_reached ();
			}
		...
		public void inject (string name, string data, Plawnekjx.Test.Arch arch = Plawnekjx.Test.Arch.CURRENT) {
			var loop = new MainLoop ();
			Idle.add (() => {
		...
		private async void perform_injection (string name, string data, Plawnekjx.Test.Arch arch, MainLoop loop) {
			if (injector == null) {
			...
			try {
				var path = Plawnekjx.Test.Labrats.path_to_library (name, arch);
				assert_true (FileUtils.test (path, FileTest.EXISTS));

				yield injector.inject_library_file (process.id, path, "plawnekjx_agent_main", data);
			} catch (GLib.Error e) {
				printerr ("\nFAIL: %s\n\n", e.message);
				assert_not_reached ();
			}
