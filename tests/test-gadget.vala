namespace Plawnekjx.GadgetTest {
		...
		private static void load_script () {
			...
			switch (Plawnekjx.Test.os ()) {
				case Plawnekjx.Test.OS.MACOS: {
					var tests_dir = Path.get_dirname (Plawnekjx.Test.Process.current.filename);
			...
			var gadget_filename = Path.build_filename (gadget_dir, "plawnekjx-gadget" + Plawnekjx.Test.os_library_suffix ());
			var config_filename = Path.build_filename (gadget_dir, "plawnekjx-gadget.config");
			...
			try {
				...
				var process = Plawnekjx.Test.Process.start (Plawnekjx.Test.Labrats.path_to_executable ("sleeper"), null, envp);
